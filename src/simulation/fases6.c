...
#define ini 1.6
	...
	int n, sumaa, *con, cien, extin, *Pi, simu, *Pf, *grad, **estat, **Ni, **pos, **Ea, i, ori, j, k, l, max, suma, gmax, ni, *Ninf, *Nedg, Aux, cepas, a, lamb, gamm;
	FILE *dades, *Pc, *Clust, *Knngraf, *Evol, *fases;
	char dad[20];
	double PC, KNN, CLUST, aux, aux2, temps, taux, GAM=0;
	bool control;
	...
	if(Ea==NULL||pos==NULL){
		...
		exit(3);};
	printf("Després de netejar la xarxe queden %d nodes i %d links \n", Aux, suma/2);
...
for(ni=1;ni<=gmax;ni++){
	...
	srand(time(NULL));
	for(gamm=0;gamm<=ranggam;gamm++){
		GAM=(ini+gamm*salt)/(double)nodos;
		extin=0;
		cien=0;
	for(simu=1;simu<=simulacions;simu++){
	for(i=0;i<=MAX;i++){
...
/*	fprintf(Evol, "0  1 \n");*/
		...
		/*	printf("%d %lf \n",cepas, temps);*/
			...
			if(Ninf[l]==0){/*S'ha extingit una cepa*/
                                ...
                                for(j=1;j<=Ninf[l];j++){
                                        Ni[l][j]=Ni[cepas][j];}
                                for(j=1;j<=Nedg[l];j++){
                                        Ea[2*l-1][j]=Ea[2*cepas-1][j];
                                        Ea[2*l][j]=Ea[2*cepas][j];};
                                for(j=1;j<=sumaa;j++){
                                        pos[l][j]=pos[cepas][j];};
	...
	if(temps>100){
		cien++;};};
	fprintf(fases, "%.15f %.15f %.15lf \n", ini+gamm*salt, cien/(double)simulacions, extin/(double)simulacions);
	printf("%.15f %.15f %.15lf \n", ini+gamm*salt, cien/(double)simulacions, extin/(double)simulacions);
/*	if(temps<LIMIT){
