...
#define LIMIT 100
	...
	int n, *con, *Pi, *Pf, *grad, **estat, **Ni, **pos, **Ea, i, ori, j, k, l, max, suma, sumaa, gmax, ni, *Ninf, *Nedg, Aux, cepas, a;
	FILE *dades, *Pc, *Clust, *Knngraf, *Evol;
	char dad[20];
	double PC, KNN, CLUST, aux, aux2, temps, taux;
	bool control;
	...
	for(i=1;i<=max;i++){
		if(grad[i]>0){
			Aux++;};};
	printf("Després de netejar la xarxa  queden %d nodes i %d links \n", Aux, suma/2);
