/*
 * Susceptibility scan for estimating the non-mutant epidemic threshold.
...
#define simu 5000
	...
	int n, *con, *Pi, *Pf, *degree, **state, **Ni, **pos, **Ea, i, simulations, seed_node, j, k, l, max, sum, gmax, ni, *Ninf, *Nedg, Aux, strains, a, lamb, recovery_count;
	FILE *input_file, *Pc, *Clust, *Knngraf, *Evol, *fases;
	char input_filename[20];
	double PC, KNN, CLUST, aux, vec[simu+1], vecua[simu+1], aux2, sim_time, event_time_increment, LAM=0, suscep, esp, espcua;
	bool control;
...
for(ni=1;ni<=gmax;ni++){
	...
	for(lamb=1;lamb<=200;lamb++){
		LAM=LAM+0.1;
		recovery_count=0;
	for(simulations=1;simulations<=simu;simulations++){
	for(i=0;i<=MAX;i++){
...
/*	fprintf(Evol, "0  1 \n");*/
	...
	vec[simulations]=recovery_count;};
	esp=0;
	espcua=0;
	for(i=1;i<=simu;i++){
